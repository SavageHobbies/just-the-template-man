...
async function testComprehensiveMock() {
  console.log('🧪 Testing Comprehensive Mock Data Processing\n');
  ...
  try {
    ...
    console.log('🚀 Starting optimization with mock data...');
    
    // Process the URL with mock data
    await cli.optimize(testUrl, { 
      output: 'comprehensive-mock-test-output.html',
      interactive: false 
    });
    ...
    if (fs.existsSync('comprehensive-mock-test-output.html')) {
      const htmlContent = fs.readFileSync('comprehensive-mock-test-output.html', 'utf8');
      ...
      // Clean up
      fs.unlinkSync('comprehensive-mock-test-output.html');
...
// Run the test
testComprehensiveMock().catch(console.error);
